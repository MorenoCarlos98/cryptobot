...
def db_store(coins, prices, market_caps, total_exchange_volumes, returns_24h, total_supply, categories, value_proposition):
    ...
    for i in range(len(coins)):
        data = {"symbol": ids[i], "name": coins[i], "price": prices[i], "market_cap": market_caps[i], "total_exchange":total_exchange_volumes[i], "return": returns_24h[i], "total_supply": total_supply[i],"category": categories[i], "value_proposition": value_proposition[i]}
        db.collection("current-cryptocoins").document(ids[i]).set(data)
    logging.debug('ALL UPDATED')    
